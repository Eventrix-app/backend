...
export class EventsController {
  ...
  async checkIn(
    ...
    // Generated by the scanning device at scan time and reused across retries, so a replayed
    // offline queue entry resolves as the success it already was instead of looking like a
    // second gate scanning the same ticket. Optional: an older client that omits it keeps
    // the previous behaviour (every repeat is treated as a conflict).
    @Body('idempotencyKey') idempotencyKey: string | undefined,
    @Request() req: Request & { user: JwtPayload },
  ) {
    return this.eventsService.checkIn(ticketCode, req.user.id, req.user.roles, idempotencyKey);
  }
